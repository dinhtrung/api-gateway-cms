...
export class ContentTypeComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.contentTypeService
      ...
      .subscribe(
        (res: HttpResponse<IContentType[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IContentType): string {
  ...
  registerChangeInContentTypes(): void {
    this.eventSubscriber = this.eventManager.subscribe('contentTypeListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IContentType[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
